...
이미지 자동 저장, 갤러리, 중복 감지, 고아 이미지 정리를 통합한 이미지 관리 플러그인입니다.

## 주요 기능

- 붙여넣기/드래그 앤 드롭 이미지 자동 저장
- 그리드/리스트 갤러리 뷰로 이미지 탐색
- SHA-256 해시 기반 중복 이미지 감지
- 노트에서 참조되지 않는 고아 이미지 탐지/정리
- 이미지 이동/이름 변경 시 링크 자동 업데이트
...
1. 설정 → 커뮤니티 플러그인을 엽니다.
2. **Star ImageMaster**를 검색합니다.
3. 설치 후 활성화합니다.

### 수동 설치

1. 최신 [GitHub 릴리스](https://github.com/starhunt/star-imagemaster/releases/latest)에서 `main.js`, `manifest.json`, `styles.css`를 다운로드합니다.
2. `<vault>/.obsidian/plugins/star-imagemaster/` 폴더를 만듭니다.
3. 다운로드한 파일을 해당 폴더에 복사합니다.
4. 앱을 다시 시작하거나 플러그인을 새로고침한 뒤 **Star ImageMaster**를 활성화합니다.
...
1. 커뮤니티 플러그인에서 **Star ImageMaster**를 활성화합니다.
